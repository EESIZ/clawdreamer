...
- openclaw에서 생성하는 에피소드 파일 로드 (`YYYY-MM-DD.md`)
- 텍스트를 의미 단위로 분할
...
- **session-memory**: 컨텍스트 윈도우가 가득 차면, 대화 요약을 에피소드 파일(`YYYY-MM-DD.md`)로 플러시하는 내부 훅.
...
2. **세션 압축**: 컨텍스트 윈도우가 차면 `session-memory` 훅이 요약을 `episodes/YYYY-MM-DD.md`에 기록
3. **매일 새벽 3시**: Dreamer가 에피소드를 읽고, 새 시맨틱 기억을 만들고, 기존 기억과 충돌을 해결하고, 오래된 기억을 정리
4. **다음 대화**: 게이트웨이가 정리된 기억을 LanceDB에서 회상
...
```
$DREAMER_HOME/
  episodes/           # 입력: 일별 마크다운 파일 (YYYY-MM-DD.md)
...
에피소드는 `YYYY-MM-DD.md` 형식의 마크다운 파일이다. AI 에이전트의 하루 경험을 자유롭게 기록한다:
